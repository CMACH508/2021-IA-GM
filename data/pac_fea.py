...
class PACObject(BaseDataset):
    def __init__(self, sets, obj_resize):
        ...
        self.classes_fea = cfg.PAC.CLASSES_FEA
        ...
        self.split_offset = cfg.PAC.TRAIN_OFFSET
        ...
        for cls_name in self.classes:
            ...
            assert ori_len > 0, 'No data found for PAC Object Class. Is the dataset installed correctly?'
    ...
    def get_pair(self, cls=None, shuffle=True):
        """
        Randomly get a pair of objects from PAC-object dataset
        ...
        if (flag):
            ...
            # print("len: ", len(self.mat_list[cls]))
            # ran_1 = random.randrange(0, len(self.mat_list[cls]), 2)
            fea_name_1 = random.choice(self.mat_list[cls])
            anno_dict_1 = self.__get_anno_dict(fea_name_1, cls)
            if shuffle:
                random.shuffle(anno_dict_1['keypoints'])
            # anno_pair.append(anno_dict_1)

            name_2 = fea_name_1.stem[:len(fea_name_1.stem)-1]
            # ff = fea_name_1.stem[len(fea_name_1.stem)-1]
            # if ff == 'a':
            #     fff = 'b.mat'
            # elif ff == 'b':
            #     fff = 'a.mat'
            fea_name_2 = fea_name_1.parent / (name_2 + 'b.mat')
            anno_dict_2 = self.__get_anno_dict(fea_name_2, cls)
            if shuffle:
        ...
        perm_mat = np.zeros_like(anno_dict_1['adj'], dtype=np.float32)
        row_list = []
        col_list = []
    ...
    def __get_anno_dict(self, mat_file, cls):
        ...
        fea_file = mat_file.parent / self.classes_fea[cls] / (mat_file.stem+'.mat')

        struct = sio.loadmat(mat_file.open('rb'))
        kpts = struct['pts_coord']
        fea = sio.loadmat(fea_file.open('rb'))['fea']
        assert fea.shape[1] == 60
        nums = fea.shape[0]
        # fea = scale(fea)
        ...
        for idx, keypoint in enumerate(np.split(kpts, kpts.shape[1], axis=1)):
            ...
            attr_ = {'name': idx}
            attr_['x'] = float(keypoint[0])
            attr_['y'] = float(keypoint[1])
            keypoints_org_list.append(attr_)
        
        fea_list = []
        for idx, key in enumerate(fea):
            att = {'name': idx}
            att['scf'] = key
            fea_list.append(att)
        ...
        anno_dict['keypoints_org'] = keypoints_org_list
        P_gt = [(kp['x'], kp['y']) for kp in anno_dict['keypoints_org']]
