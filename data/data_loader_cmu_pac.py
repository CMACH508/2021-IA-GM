...
import numpy as np
import random
from data.cmu_hotel_house import CmuObject
from data.car_bikes import PACObject
...
class GMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        n1_gt, n2_gt = len(anno_pair[0]['fea']), len(anno_pair[1]['fea'])
        
        ret_dict = {'As': [torch.Tensor(x) for x in [anno_pair[0]['adj'], anno_pair[1]['adj']]],
...
class DGMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        P1_gt = [(kp['x'], kp['y']) for kp in anno_pair[0]['keypoints_obj']]
        P2_gt = [(kp['x'], kp['y']) for kp in anno_pair[1]['keypoints_obj']]

        assert len(P1_gt) == n1_gt, len(P2_gt) == n2_gt
        ...
        imgs = [anno['image_obj'] for anno in anno_pair]
