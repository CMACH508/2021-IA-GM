...
class PacObject(BaseDataset):
    def __init__(self, sets, obj_resize):
        ...
        for cls_name in self.classes:
            ...
            cls_mat_list = [p for p in (self.root_path / cls_name).glob('*a.mat')]
            cls_mat_list = sorted(cls_mat_list)
            ...
            if self.split_offset % ori_len + self.train_len <= ori_len:
                ...
                # if sets == 'train':
                #     self.mat_list.append(
                #         cls_mat_list[:(self.split_offset-1) % ori_len] +
                #         cls_mat_list[(self.split_offset):]
                #     )
                # else:
                #     self.mat_list.append(
                #         cls_mat_list[(self.split_offset-1):self.split_offset]
                #     )
            else:
                if sets == 'train':
                    self.mat_list.append(
    ...
    def get_pair(self, cls=None, shuffle=True):
        ...
        if (flag):
            ...
            if ff == 'a':
                ...
                fff = 'a.mat'
            fea_name_2 = fea_name_1.parent / (name_2 + fff)
