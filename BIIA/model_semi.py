...
class Net(CNN):
    def __init__(self):
        ...
        # resnet
        self.R = nn.Linear(cfg.BIIA.FEATURE_CHANNEL * 2, cfg.BIIA.FEATURE_CHANNEL * 2, bias=False)
        self.relu = nn.ReLU()

        # self.affinity = Affinity(cfg.BIIA.FEATURE_CHANNEL)
        # self.cosine_affinity = Cosine_Affinity(cfg.BIIA.FEATURE_CHANNEL)
        self.merge_layer = MergeLayers(cfg.BIIA.FEATURE_CHANNEL * 2).cuda()
        self.bi_stochastic = Sinkhorn(max_iter=cfg.BIIA.BS_ITER_NUM, epsilon=cfg.BIIA.BS_EPSILON)
        # self.voting_layer = Voting(alpha=cfg.BIIA.VOTING_ALPHA)
        self.voting_layer_0 = FullPro(alpha=cfg.BIIA.VOTING_ALPHA)
        self.voting_layer_1 = FullPro(alpha=(cfg.BIIA.VOTING_ALPHA/2))
        self.hung = hungarian
        self.l2norm = nn.LocalResponseNorm(cfg.BIIA.FEATURE_CHANNEL*2, alpha=cfg.BIIA.FEATURE_CHANNEL*2, beta=0.5, k=0)
        self.layernorm = nn.LayerNorm(torch.randn(8, 30, cfg.BIIA.FEATURE_CHANNEL).size()[1:])

        for i in range(self.gnn_layers):
            ggnn = Siamese_Net(cfg.BIIA.FEATURE_CHANNEL*2)
            self.add_module('ggnn_{}'.format(i), ggnn)

        for k in range(self.iteration_):
            self.add_module('affinity_{}'.format(k), Affinity(cfg.BIIA.FEATURE_CHANNEL*2))
